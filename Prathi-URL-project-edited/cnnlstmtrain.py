...
checkpointer = callbacks.ModelCheckpoint(filepath="logs/cnnlstm/checkpoint-{epoch:02d}.hdf5", save_best_only=True, monitor='val_acc', mode='max')
csv_logger = CSVLogger('logs/cnnlstm/cnnlstmanalysis.csv',separator=',', append=False)
model.fit(X_train, y_train, batch_size=32, nb_epoch=60,validation_split=0.2, shuffle=True,callbacks=[checkpointer,csv_logger])
model.save("logs/cnnlstm/completemodel.hdf5")
'''
